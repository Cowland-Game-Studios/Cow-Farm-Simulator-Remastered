...
import React, { useState, useRef, useCallback, useEffect, useMemo, MouseEvent } from "react";
...
import {
    ...
    InventorySidebar,
    RecipeSidebar,
    PlacedIngredientData,
    TimedCraftingData,
    RemovingIngredientData,
    SelectedIngredient,
} from './components';
...
export default function Crafting({ onClose = () => {} }: CraftingProps): React.ReactElement {
    // ============================================
    // CONTEXT & STATE
    // ============================================
    ...
    const [ingredientsPlaced, setIngredientsPlaced] = useState<PlacedIngredientData[]>([]);
    ...
    const [timedCrafting, setTimedCrafting] = useState<TimedCraftingData | null>(null);
    const [timedCraftingRemaining, setTimedCraftingRemaining] = useState(0);
    
    // Removing ingredient animations
    const [removingIngredients, setRemovingIngredients] = useState<RemovingIngredientData[]>([]);
    
    // Recipe pulse animation tracking
    const [pulsingRecipes, setPulsingRecipes] = useState<Set<string>>(new Set());
    const prevBoardCraftableRef = useRef<Set<string>>(new Set());
    ...
    const selectedIngredientRef = useRef<SelectedIngredient | null>(null);
    const ingredientsPlacedRef = useRef<PlacedIngredientData[]>([]);
    ...
    const sortedRecipes = useMemo(() => {
        return sortRecipes(RECIPES, boardIngredientCounts, inventory);
    }, [boardIngredientCounts, inventory]);
    ...
    const spawnIngredientsOnBoard = useCallback((recipe: Recipe) => {
        const newIngredients: PlacedIngredientData[] = [];
    ...
    const removePlacedIngredient = useCallback((ingredient: PlacedIngredientData) => {
        ...
        requestAnimationFrame(() => {
            ...
            setRemovingIngredients(prev => prev.map(r => 
                r.id === removeId ? { ...r, phase: 'flyout' as const } : r
            ));
        });
