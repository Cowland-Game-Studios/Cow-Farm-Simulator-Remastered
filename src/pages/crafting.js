...
export default function Crafting({ onClose = () => {} }) {
    ...
    // Timed crafting state (timestamp-based for persistence)
    ...
    useEffect(() => {
        timedCraftingRef.current = timedCrafting;
    }, [timedCrafting]);
    ...
    useEffect(() => {
        if (activeBoardCraft && !timedCrafting) {
            ...
            if (recipe) {
                ...
                // Set output target position after DOM renders
    ...
    // Spawn ingredients from inventory onto the crafting board with lerp animation
    const spawnIngredientsOnBoard = useCallback((recipe) => {
        ...
        for (const input of recipe.inputs) {
            for (let i = 0; i < input.qty; i++) {
                ...
                // Find the source position (sidebar item)
                const sourceElement = document.querySelector(`[data-item-id="${input.item}"]`);
                let sourcePos = { x: 100, y: window.innerHeight / 2 }; // Default fallback
                if (sourceElement) {
                    const rect = sourceElement.getBoundingClientRect();
                    sourcePos = { x: rect.left + rect.width / 2, y: rect.top + rect.height / 2 };
                }
                
                // Deduct from inventory
                removeItem(input.item, 1);
                
                // Add to board with spawn animation data
                // Phase 1: Start at source position
                newIngredients.push({
                    name: input.item,
                    image,
                    x: targetPos.x,
                    y: targetPos.y,
                    // Animation: start at source, will lerp to target
                    currentX: sourcePos.x,
                    currentY: sourcePos.y,
                    spawnPhase: 'starting', // 'starting' -> 'animating' -> 'done'
                    spawnId: Date.now() + Math.random(), // Unique ID for this spawn
                });
            }
        }
        
        // Add ingredients at their spawn position
        setIngredientsPlaced(prev => [...prev, ...newIngredients]);
        
        // Phase 2: Start animation to target (after first render)
        requestAnimationFrame(() => {
            requestAnimationFrame(() => {
                setIngredientsPlaced(prev => 
                    prev.map(ing => {
        ...
        // Phase 3: Animation complete, spawn particles
        setTimeout(() => {
            setIngredientsPlaced(prev => 
                prev.map(ing => {
                    if (ing.spawnPhase === 'animating') {
                        // Spawn particle at final position
    ...
    const handleCraft = useCallback((recipe) => {
        // Don't allow crafting during animation or timed crafting
        ...
        if (canCraftFromBoard) {
            ...
            // Check if this is a timed recipe
            ...
            setTimeout(() => {
                ...
                // For timed recipes, set up the timer-based completion
                if (isTimedRecipe) {
                    const craftData = {
                        startedAt: Date.now(),
                        duration: recipe.time * 1000, // Convert to ms
                        recipe,
                        ingredientIds: usedIngredientIndices,
                    };
                    setTimedCrafting(craftData);
                    setTimedCraftingRemaining(recipe.time * 1000);
                    
                    // Save to global state for persistence
                    // Get the current ingredients that are being crafted
                    const craftingIngredients = ingredientsPlaced.filter((_, idx) => 
                        usedIngredientIndices.includes(idx)
                    ).map(ing => ({
                        name: ing.name,
                        image: ing.image,
                        x: window.innerWidth / 2, // They're at center during spinning
            ...
            // For timed recipes, the timer effect handles phases 3-5
    ...
    // Handle click on placed ingredient - single click to remove
            ...
            {ingredientsPlaced.map((ingredient, index) => {
                ...
                return (
                    <button
                        ...
                        onClick={(e) => !isBeingCrafted && handlePlacedIngredientClick(e, ingredient)}
                    >
